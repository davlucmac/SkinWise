...
let PRODUCTS = [];
const CATALOG_URL = "./data/products.catalog.json";
...
function renderResults(scored, intake) {
  ...
  scored.slice(0, 5).forEach((p, idx) => {
    ...
    node.innerHTML = `
      ...
      <p><strong>PDP URL:</strong> <a href="${p.pdpUrl || "#"}" target="_blank" rel="noopener noreferrer">${p.pdpUrl || "Not provided"}</a></p>
...
async function loadCatalog() {
  try {
    ...
    if (!Array.isArray(payload) || payload.length === 0) throw new Error("Empty catalog");
    PRODUCTS = payload;
  } catch (error) {
    console.warn("Falling back to bundled products catalog", error);
